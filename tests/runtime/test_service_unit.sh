...
assert_contains "$unit" "WantedBy=default.target" "설치 대상"

assert_eq "$(grep -c '^\[Service\]$' "$U")" "1" "[Service] 정확히 하나"
assert_eq "$(grep -c '^ExecStart=' "$U")" "1" "ExecStart 정확히 하나"

# 유닛이 quickshell 을 직접 부르면 안 된다 — 기동 명령은 래퍼가 소유한다.
assert_eq "$(grep -c 'quickshell' "$U")" "0" "유닛은 quickshell 을 직접 부르지 않는다"
# 컴포지터를 띄우려 하면 안 된다.
