#!/usr/bin/env bash
# --restart 는 세션이 잠긴 동안 셸을 죽이면 안 된다. lock 상태를 IPC 로
# 물어보고 .secure/.requested 가 참이면 거부, IPC 자체가 실패하면(보존할
# 락이 없으므로) 진행, 응답은 왔는데 파싱이 안 되면 안전하게 거부한다.
...
# (a) secure:true → 거부
out=$(run_restart '{"secure":true,"requested":false}' 0 2>&1); code=$?
...
# (b) requested:true → 거부
out=$(run_restart '{"secure":false,"requested":true}' 0 2>&1); code=$?
...
# (c) secure:false, requested:false → 진행 (setsid 호출)
out=$(run_restart '{"secure":false,"requested":false}' 0 2>&1); code=$?
