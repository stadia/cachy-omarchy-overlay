...
The upstream theme pipeline is used as-is. The first `cachy-omarchy-init` seeds
...
are loaded by default per upstream's own rules. The helpers their QML calls and the
runtime dependencies they need (`jq`, `wl-clipboard`, `wtype`, `wireplumber`,
`pipewire-pulse`, `xdg-utils`) are packaged alongside them.
