...
| reboot / shutdown | 핵심 | host | 미검증 | 마지막 남은 행. 세션을 끝내므로 재부팅 뒤 저널에서 수습한다. **재부팅 직전 기준값(2026-08-24 17:20):** boot_id 가 바뀌었는지, `uptime -s` 가 갱신됐는지, 셸 PID 926740 이 새 PID 로 교체됐는지, `~/.config/omarchy/plugins` 감시 `inotifywait` 고아 2개가 새 부팅에서 0 으로 초기화됐는지(reload 누수는 부팅 경계를 넘지 않아야 한다), 테마 `tokyo-night` 과 배경 `0-winding-road.jpg` 가 보존됐는지를 본다. **수습할 증거:** 종료 측에서 `Stopped target Session envelope of hyprland.desktop` 과 uwsm RC 0, `Reached target Shutdown`; 복귀 측에서 세션 안 doctor exit 0, `hyprctl layers` 의 두 서피스, IPC ping, journal QML 오류 부재. |
